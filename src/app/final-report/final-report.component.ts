...
export class FinalReportComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    /*
    try {
    this.image64geoserver = 
      await this.getBaseImageUrl(
        `http://www.terrama2.dpi.inpe.br/mpmt/geoserver/wms?service=WMS&version=1.1.0&request=GetMap&layers=terrama2_5:view5&styles=` +
        `&bbox=${this.property?.bbox}` +
        `&width=404&height=431` +
        `&time=${this.dateFilter}` +
        `&cql_filter=numero_do1='${this.property?.register}'` +
        `&srs=EPSG:4674&format=image/png`
      )
    } catch(e) {
      this.image64geoserver = 
        await this.getBaseImageUrl(
          'https://st2.depositphotos.com/3974537/6197/v/600/depositphotos_61975307-stock-video-tv-static-noise-signal.jpg'
        )
    }
    */
    ...
    this.imageHeader = await this.getBaseImage('assets/img/logos/logo-mpmt2.png');
  }
